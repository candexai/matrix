...
export function ElevenLabsConnectDialog({ open, onOpenChange, mode = "connect" }: { open: boolean; onOpenChange: (o: boolean) => void; mode?: ConnectDialogMode }) {
  ...
  const title = mode === "replace" ? "Replace Candex key" : usingEnv ? "Add your own Candex key" : "Connect Candex";
    ...
    <Dialog open={open} onOpenChange={(o) => (!connect.isPending ? onOpenChange(o) : undefined)}>
      <DialogContent size="md">
        <DialogHeader>
          ...
          <DialogDescription>Pilot runs agents, voices, phone numbers and calls through your own Candex account. The key is stored encrypted and used only for this workspace.</DialogDescription>
        </DialogHeader>
        <DialogBody className="space-y-5">
          <div className="rounded-lg border border-border bg-muted/40 p-4 text-[13px] leading-6">
            <div className="mb-1 font-medium">Where do I find my key?</div>
            <p className="text-muted-foreground">Your Candex voice API key comes with your Candex plan. If you don’t have one yet, ask your Candex account manager — it needs access to agents, voices, phone numbers and usage.</p>
          </div>
          ...
          {mode === "replace" && !usingEnv ? (
            <p className="text-[13px] text-muted-foreground">Agents and numbers stay as they are if the new key belongs to the same Candex account. A key from a different account shows that account’s agents instead.</p>
          ) : null}

          <Field label="API key" htmlFor="eleven-api-key" error={invalidKey ? error?.message || "Candex rejected this key." : undefined} help={invalidKey ? undefined : "Paste the whole key — recent ones start with sk_."}>
            <SecretInput
              id="eleven-api-key"
              value={apiKey}
          ...
          <Field label="Region" htmlFor="eleven-region" help="Where your Candex account lives. Pick EU only if you signed up with EU data residency.">
