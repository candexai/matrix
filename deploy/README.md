...
- **Call tags (AI insights)**: rules live in `backend/src/services/insights.service.ts` (`buildInsightsPrompt`). After changing them, preview on real calls without writing anything: `cd backend && npx ts-node --transpile-only src/scripts/insightsDryRun.ts --n 60` (run from a machine with the repo; it only reads). Then rebuild the vocabulary and re-tag every call: `cd /opt/matrix/backend && node dist/scripts/reanalyzeInsights.js --reset --days 365 --max 1000` (add `--workspace <id>` for one workspace). Vocabulary size: `INSIGHT_TAG_CAP` in `backend/.env` (default 30).
