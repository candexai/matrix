...
/**
 * Content of the "Call history" tab: every call with this caller (timeline) and what those calls taught us,
 * next to a compact caller card (who it is, contact, relationship stats, CRM fields, Save as lead).
 * Two columns when the tab is wide enough, stacked otherwise. Works for a linked lead, an unknown phone
 * caller and a browser call.
 */
export function LeadProfilePanel({ conversationId, onSelectConversation }: { conversationId: string; onSelectConversation: (id: string) => void }) {
  ...
  if (isLoading) return <ProfileSkeleton className="max-w-[520px]" />;
  if (isError || !p) return <ProfileError message={isError ? errorMessage(error) : "No call history is available for this conversation."} onRetry={() => refetch()} retrying={isFetching} />;

  return (
    <div className="@container min-w-0 pb-2">
      <div className="grid gap-x-10 gap-y-8 @[760px]:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="min-w-0 space-y-7 @[760px]:border-l @[760px]:border-border @[760px]:pl-8" aria-label="Caller">
